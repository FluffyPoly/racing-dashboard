...
export default function RaceCard({ race }: { race: any }) {
  ...
  return (
    <div className="racing-card p-6 rounded-r-xl shadow-sm hover:shadow-md transition-all border border-gray-100 h-fit">
      <div className="flex justify-between items-start mb-6">
        ...
        <div className="flex flex-col items-end gap-1">
          ...
          <span className="text-[9px] text-gray-300 font-mono italic">#{race.race_id}</span>
        </div>
      </div>
      ...
      <div className="mt-8 pt-6 border-t border-gray-100 flex justify-between items-center">
        <div className="flex gap-3">
          <div className="flex items-center gap-1 group relative">
            <BarChart3 size={16} className={race.analysis.keenan ? "text-gold" : "text-gray-200"} />
          ...
          <div className="flex items-center gap-1 group relative">
            <Zap size={16} className={race.analysis.mordin ? "text-gold" : "text-gray-200"} />
            <span className="absolute -top-8 left-0 bg-gray-800 text-white text-[10px] px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap">
              Market Calibrated
            </span>
          </div>
                  ...
                  <div>
                    ...
                    {race.analysis.keenan.forecast_top_3?.slice(0, 2).map((f: any) => (
                      <p key={f.combination} className="text-xs font-bold text-gray-700 leading-tight mb-1">
                        {f.combination.replace('>', '→')} <span className="text-gold italic">{Math.round(f.probability*100)}%</span>
                      </p>
                    ))}
                  </div>
                  <div>
                  ...
                  {race.analysis.mordin.market_comparison?.map((m: any) => (
                    <div key={m.horse} className="flex justify-between items-center text-xs">
                      ...
                      <span className={`font-black italic ${
                        m.divergence > 0.05 ? 'text-green-600' : m.divergence < -0.05 ? 'text-red-500' : 'text-gray-400'
                      }`}>
                        {m.divergence > 0 ? '+' : ''}{Math.round(m.divergence * 100)}%
                      </span>
                    </div>
                  ))}
                </div>
